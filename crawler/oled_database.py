...
class OledDatabase(Database) :
        ...
        with self.conn.cursor() as cur :
            for eval_result in eval_results :
                ...
                try :
                    ...
                    try :
                        eval_result['name'] = eval_result['name'].replace('\uff1f', '').replace('\uff1a', '').replace('\u3000', '')
                        eval_result['no'] = eval_result['no'].replace('\uff1f', '').replace('\uff1a', '').replace('\u3000', '').replace('Bidding NO', '').strip()
                        eval_result['content'] = eval_result['content'].replace('\uff1f', '').replace('\uff1a', '').replace('\u3000', '')
                        eval_result['agency'] = eval_result['agency'].replace('\uff1f', '').replace('\uff1a', '').replace('\u3000', '')
                        eval_result['purchasers'] = eval_result['purchasers'].replace('\uff1f', '').replace('\uff1a', '').replace('\u3000', '')
                        eval_result['open_time'] = eval_result['open_time'].replace('\uff1f', '').replace('\uff1a', '').replace('\u3000', '')
                        eval_result['ending_date'] = eval_result['ending_date'].replace('\uff1f', '').replace('\uff1a', '').replace('\u3000', '')

                        cur.execute(sql, (eval_result['name'], eval_result['no'], eval_result['content'],
                                    eval_result['agency'], eval_result['purchasers'],
                                    eval_result['open_time'], eval_result['ending_date']))
    ...
    def select_eval_results(self, purchaser, date)  :
        sql = '''select * from EVAL_RESULTS where PURCHASERS="%s" and INSERT_DATE>="%s"''' % (purchaser, date)
        eval_results = []
        print(sql)
        with self.conn.cursor() as cur :
            cur.execute(sql)
    ...
    def select_tender_awards(self, purchaser, date) :
        sql = '''select PROJECT_NAME, BIDDING_NO,
                BIDDING_CONTENT, BIDDING_AGENCY, PURCHASERS, OPEN_TIME,
                EVAL_RESULT_DATE, BIDDING_RESULT_DATE, FINAL_WINNER,
                MANUFACTURER, COUNTRY from TENDER_AWARDS
                where PURCHASERS="%s" and INSERT_DATE>="%s"''' % (purchaser, date)
        tender_awards = []
    ...
    def select_all_tender_awards(self) :
        sql = '''select PROJECT_NAME, BIDDING_NO,
                BIDDING_CONTENT, BIDDING_AGENCY, PURCHASERS, OPEN_TIME,
                EVAL_RESULT_DATE, BIDDING_RESULT_DATE, FINAL_WINNER,
                MANUFACTURER, COUNTRY from TENDER_AWARDS'''
