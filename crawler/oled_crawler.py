...
class OledCrawler(BaseCrawler) :
    key_word_dict = {'CSOT' : (2, 11), 'BOE' : (3, 28), 'Tianma' : (1, 9),
                     'Optoelectronics' : (4, 47), 'CEC' : (3, 17),
                     'Optronics' : (1, 2), 'Yungu' : (2, 12), 'Royole' : (1, 8)}
        ...
        if len(self.results_soup_list) == 0 or len(self.awards_soup_list) == 0:
            ...
            for results_soup in self.results_soup_list :
                ...
                for list_item in list_items :
                    ...
                    #print(result_time)
                    if result_time < "Time：2017-05-01" :
                        break
    ...
    def __get_data_from_links(self, link_list, data_list) :
        for link in link_list :
            ...
            if raw_data :
                refined_data = self.__refine_data(raw_data)
